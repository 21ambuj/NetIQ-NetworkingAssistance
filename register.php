...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($confirm_password) || empty($age) || empty($gender)) {
        $error = "Please fill in all required fields.";
    } elseif ($password != $confirm_password) {
        $error = "Passwords do not match.";
    } else {
        $stmt = $conn->prepare("SELECT id FROM useraii WHERE email = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO useraii (username, email, password, age, gender, agree) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssisi", $username, $email, $hashed_password, $age, $gender, $agree);
            if ($stmt->execute()) {
